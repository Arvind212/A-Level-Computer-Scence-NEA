...
from CreateClass import Ui_ClassWindow
...
class Ui_TeacherMainWindow(object):
    def setupUi(self, TeacherMainWindow):
        ...
        TeacherMainWindow.resize(1200, 800)
        TeacherMainWindow.setWindowTitle("Teacher Main Window")
        TeacherMainWindow.setStyleSheet("QMainWindow#TeacherMainWindow{background-color: rgb(24, 24, 24);}")
...
"QPushButton#calcsButton:pressed{background-color: rgb(170, 170, 170);}")
        ...
        cur.execute("SELECT title,surname from tblTeacherUsers WHERE Logged_in = 1")
        items = cur.fetchone()
        title = items[0]
        surname = items[1]
        connection.commit()
        connection.close()
        self.welcomeLabel.setText(f"Welcome {title} {surname}!")
...
"QPushButton#SettingsButton:pressed{background-color: rgb(170, 170, 170);}")
        ...
        self.classesButton = QtWidgets.QPushButton(self.centralwidget, clicked=lambda: self.openCreateClass())
